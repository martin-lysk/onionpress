...
#
# Prerequisites:
#   - OnionPress running locally with ONIONHEAVEN=1
#   - Docker accessible
#
# Usage:
...
ONION_ADDR=$(docker exec "$TOR_CONTAINER" cat /var/lib/tor/hidden_service/hostname 2>/dev/null || echo "")
...
if docker ps --format '{{.Names}}' | grep -q "^onionheaven$"; then
    log "  onionheaven container is already running (prior activation)"
else
...
step 4 "Verify takeover propagation (Arti keystore + config)"

# Check that the key was installed in Arti's keystore
ADDR_PREFIX="${CONTENT_ADDR:0:20}"
ARTI_KEY_EXISTS=$(docker exec "$TOR_CONTAINER" sh -c "ls /var/lib/arti/state/keystore/hss/onionheaven_${ADDR_PREFIX}*/ks_hs_id* 2>/dev/null | head -1" || echo "")
if [ -n "$ARTI_KEY_EXISTS" ]; then
    pass "Key installed in Arti keystore: $(basename $(dirname $ARTI_KEY_EXISTS))"
else
    # Try finding by content address pattern
    ARTI_KEY_EXISTS=$(docker exec "$TOR_CONTAINER" sh -c "find /var/lib/arti/state/keystore/hss/ -name 'ks_hs_id*' 2>/dev/null | grep -i '$(echo $CONTENT_ADDR | cut -c1-10)' | head -1" || echo "")
    if [ -n "$ARTI_KEY_EXISTS" ]; then
        pass "Key installed in Arti keystore"
    else
        fail "Key not found in Arti keystore"
        log "  Listing keystore:"
        docker exec "$TOR_CONTAINER" sh -c "ls -la /var/lib/arti/state/keystore/hss/ 2>/dev/null" || true
    fi
fi
...
step 6 "Verify release propagation (Arti keystore cleaned up)"

# Give Arti a moment to process the SIGHUP
sleep 5

# Check that the key was removed from Arti's keystore
ARTI_KEY_GONE=$(docker exec "$TOR_CONTAINER" sh -c "find /var/lib/arti/state/keystore/hss/ -path '*${ADDR_PREFIX}*' 2>/dev/null | head -1" || echo "")
if [ -z "$ARTI_KEY_GONE" ]; then
    pass "Key removed from Arti keystore"
else
    fail "Key still present in Arti keystore: $ARTI_KEY_GONE"
fi
